...
void part1() {
	...
	printf("Number of bytes allocated for s1 is: %d\n", s1_len);
	...
	printf("Number of bytes allocated for s2 is: %d\n", s2_len);

	printf("===== Question 6 =====\n");
	char *s3 = s1;
	int s3_len = sizeof(s3);
	printf("Number of bytes allocated for s3 is: %d\n", s3_len);	

	printf("===== Question 7 =====\n");
	char *s4 = malloc(1024 * sizeof(char));
	int s4_len = sizeof(s4);
	free(s4);
	printf("Number of bytes allocated for s4 is: %d\n", s4_len);
...
void part2() {
	...
	if(&s1[0]==&s3[0]){
		printf("s1: %lu == s3: %lu\n",(long unsigned int)s1,(long unsigned int)s3);
	}
	if(&s2[0]==&s3[0]){
		printf("s2: %lu == s3: %lu\n",(long unsigned int)s2,(long unsigned int)s3);
	}
	printf("===== Question 15 =====\n");
	if(strcmp(&s1[0],&s2[0]) == 0){
		printf("s1: %lu == s2: %lu\n",(long unsigned int)s1,(long unsigned int)s2);
	}
	if(strcmp(&s1[0],&s3[0]) == 0){
		printf("s1: %lu == s3: %lu\n",(long unsigned int)s1,(long unsigned int)s3);
	}
	if(strcmp(&s2[0],&s3[0]) == 0){
		printf("s2: %lu == s3: %lu\n",(long unsigned int)s2,(long unsigned int)s3);
	}
...
void part3() {
	...
	printf("===== Question 19 =====\n");
	if(memcmp(&s1[0],&s2[0], 6) == 0){
		printf("s1: %lu == s2: %lu\n",(long unsigned int)s1,(long unsigned int)s2);
	}
	if(memcmp(&s1[0],&s3[0], 6) == 0){
		printf("s1: %lu == s3: %lu\n",(long unsigned int)s1,(long unsigned int)s3);
	}
	if(memcmp(&s2[0],&s3[0], 6) == 0){
		printf("s2: %lu == s3: %lu\n",(long unsigned int)s2,(long unsigned int)s3);
	}
}
...
void part5(char *filename) {
	...
	totread += nread;
	printf("nread contents: %ld\n", nread);
	printf("totread contents: %ld\n", totread);
	...
	totread += nread;
	printf("nread value: %ld\n", nread);
	printf("totread value: %ld\n", totread);
	memprint(buf, "%02x", BUFSIZE);
	...
	printf("===== Question 44 =====\n");
	printf("Return value of closeing fd1: %d\n", close(fd1));
